...
class Export(object):
    def __init__(self, dirs_incl_list,dir_lib, defs_list):
        ...
        if not isinstance(dir_lib,Directory): raise Exception("Exported library directory must be an instance of \"nobs.Directory\"!")
        self.dir_lib = dir_lib
...
class _TargetBase(object):
    ...
    DYNAMIC_LIBRARY = 1
    EXECUTABLE = 2
    def __init__(self, name, type, dependencies_list, pch):
...
class _TargetLibraryBase(_TargetBase):
    def __init__(self, name, type, exported, dependencies_list, pch):
        _TargetBase.__init__(self, name, type, dependencies_list, pch)
...
class TargetStaticLibrary(_TargetLibraryBase,_TargetUserBase):
    def __init__(self, name, exported, headers_list,sources_list, dependencies_list, pch=None):
        _TargetLibraryBase.__init__(self, name, _TargetBase.STATIC_LIBRARY, exported, dependencies_list, pch)
...
class TargetDynamicLibrary(_TargetLibraryBase,_TargetUserBase):
    def __init__(self, name, dirs_inc_provided,dir_lib_provided, headers_list,sources_list, dependencies_list, pch=None):
        _TargetLibraryBase.__init__(self, name, _TargetBase.DYNAMIC_LIBRARY, dirs_inc_provided,dir_lib_provided, dependencies_list, pch)
        _TargetUserBase.__init__(self, headers_list,sources_list)
...
class TargetExecutable(_TargetBase,_TargetUserBase):
    def __init__(self, name, headers_list,sources_list, dependencies_list, pch=None):
        _TargetBase.__init__(self, name, _TargetBase.EXECUTABLE, dependencies_list, pch)
        _TargetUserBase.__init__(self, headers_list,sources_list)
