...
class BuildOptions(object):
	...
	def __init__(self, basic_type):
		...
		if   basic_type == "debug":
			...
			raise Exception("Unrecognized basic type \""+str(basic_type)+"\"!  Supported types are { \"debug\", \"release\", \"release-with-symbols\" }.")
...
##        clang: also "-fsanitize=memory", "-fsanitize=integer"
	...
	def setStdLibDynamic(self):
		self.is_stdlib_dynamically_linked = True
	def setStdLibStatic (self):
		self.is_stdlib_dynamically_linked = False

	def addDefine(self, define):
		if not isinstance(define,Define): raise Exception("Define must be an instance of `nobs.Define`!")
